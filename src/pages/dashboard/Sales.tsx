...
export default function Sales() {
  ...
  const handleSubmitSale = async () => {
    ...
    const invoice_number = invoicePreview;
    ...
    for (const c of cart) {
      ...
      await supabase.from("inventory_transactions").insert({ product_id: c.product_id, type: "OUT", quantity: c.quantity, note: `Penjualan ${invoice_number}` });
    }

    // Upsert customer: find by phone if provided, else by name
    ...
    if (customerId) {
      await supabase.from("sales").update({ customer_id: customerId } as any).eq("id", sale.id);
    }

    toast({ title: "Penjualan berhasil dicatat!", description: `Invoice ${invoice_number}` });
    resetForm();
    setOpen(false);
    load();
  };
